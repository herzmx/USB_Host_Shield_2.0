...
uint8_t XBOXUSB::Init(uint8_t parent, uint8_t port, bool lowspeed) {
        ...
        if(VID != XBOX_VID && VID != MADCATZ_VID && VID != JOYTECH_VID && VID != GAMESTOP_VID && VID != HORI_VID && VID != HORI_FIGHTING_EDGE_VID) // Check VID
...
#endif
                goto FailUnknownDevice;
        } else if(PID != XBOX_WIRED_PID && PID != MADCATZ_WIRED_PID && PID != GAMESTOP_WIRED_PID && PID != AFTERGLOW_WIRED_PID && PID != JOYTECH_WIRED_PID && PID != HORI_SOUL_CALIBUR_PID && PID != HORI_FIGHTING_EDGE_PID) // Check PID
                goto FailUnknownDevice;
